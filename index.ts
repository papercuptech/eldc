...
export default function defineContext<T>(props:T = <any>{}, hooks:IHooks = {}):(T & IContext<T>) {
	const create = hooks.create || function(props, parent) {
		const newInstance =  Object.create(props)
	...
	function Context(...args) {
		...
		*/
		...
		newContext.id = id++
		current && current.forEach((instance, Context) => newContext.set(Context, instance))

		const parent = newContext.get(Context) || Context[symLoopGlobal]
		const newInstance = create.call(Context, init, parent)
		newContext.set(Context, newInstance)

		function run(fn) {
			const from = current
			try{
				plg(`${cur()} -> ${newContext ? newContext.id : '*'}`)
				switchTo(newContext);	
				const result = fn()
				plg(`${from ? from.id : '*'} <- ${newContext ? newContext.id : '*'}`)
				switchTo(from)
				return result
			}
			catch(ex) {
	...
	Context[symEnter] = hooks.enter

	Context[symLoopGlobal] = create.call(Context, props)
	hooks.top && hooks.top.call(this, Context[symLoopGlobal])
	...
	Object.getOwnPropertyNames(props).forEach(name => {
		Object.defineProperty(Context, name, {
			...
			get() {
				...
				const instance = current.get(Context)
				if(!instance) return Context[symLoopGlobal][name]
				return instance[name]
			},
			set(v) {
				if(!current) return Context[symLoopGlobal][name] = v
				const instance = current.get(Context)
				if(!instance) return Context[symLoopGlobal][name] = v
				instance[name] = v
			}
		})
	})
