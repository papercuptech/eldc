...
;(() => {
	C(async function ctxTwo () {
		...
		try {
			...
			const gen = tick('Two', 20, 4)
			//plg(Context.Current.id, 'after tick')
			...
			const on = pm.then
			pm.then = function() {console.log('hmm'); return on.apply(this, arguments)}
			plg(Context.Current.id, 'after tick nap/next')

			await pm
			//await gen.next()

			plg(Context.Current.id, 'after tick.next() 1')

			//const pm2 = gen.next()
			//await pm2
			//await gen.next()
			//plg(Context.Current.id, 'after tick.next() 2')
			...
			//await nap(10, 'Two')
			//plg(Context.Current.id, 'after await 1')

			//await nap(10, 'Two')
			//plg(Context.Current.id, 'after await 2')
