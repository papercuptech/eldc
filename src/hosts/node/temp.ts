...
function makeFrameN(callAddress, address, n) {
	...
	return function(frame, args) {
		...
		args[n] = frameContinuation(address, cb, isPerCall)
	}
}
...
function makeFrameNx(address, isPerCall, n) {
	return function(frame, args) {
		const cb = args[n]
		if(!cb || !isFn(cb)) return
		args[n] = frameContinuation(address, cb, isPerCall)
...
function makeFrameZerox(address, barrier) {return makeFrameNx(address, barrier.isPerCall, 0)}
...
function makeFrameLast(callAddress, address) {
	...
	return function(frame, args) {
		...
		const cb = args[l]
		if(!cb || !isFn(cb)) return
		args[l] = frameContinuation(address, cb, isPerCall)
	}
}



function frameObject(props, obj, isPerCall, isProxy) {
	if(!obj) return
	let idx = props.length
	const proxy = isProxy ? create(obj) : obj
	const desc:any = {configurable: true, enumerable: true}
	while(idx--) {
		const {name, address} = props[idx]
		desc.get = function() {return passBack(frameContinuation(address, obj[name], isPerCall))}
...
function makeFrameRegisterElement($) {
	const props = [
		{name: 'createdCallback', address: $.createdCallback()},
		{name: 'attachedCallback', address: $.attachedCallback()}
	]
	return function(frame, args) {
		const arg1 = args[1]
		const prototype = arg1 && arg1.prototype
		if(!prototype) return
		frameObject(props, prototype, true, true)
	}
}
...
descriptors[g.Promise.prototype.then[sid]].frameArgs =0
function makeFramePromiseThen($, barrier) {
	//const onfulfilled =  $.onfulfilled
	//const onrejectedSid = $.onrejected
	return function framePromiseThen(frame, args) {
		const onfulfilled = args[0]
		const onrejected = args[1]
		if(onfulfilled && isFn(onfulfilled))
			args[0] = frameContinuation($.onfulfilled, onfulfilled, true, true)
		if(onrejected && isFn(onrejected))
			args[1] = frameContinuation($.onrejected, onrejected, true, true)
	}
}
...
descriptors[g.Promise.prototype.catch[sid]].frameArgs = function makeFramePromiseCatch() {
	...
	return function framePromisCatch(frame, args) {
		const onrejected = args[0]
		if(onrejected && isFn(onrejected))
			args[1] = frameContinuation(onrejectedSid, onrejected, true, true)
	}
}()
...
descriptors[g.Promise.prototype.finally[sid]].frameArgs = function makeFramePromiseFinally() {
	...
	return function(frame, args) {
		...
		if(onfinally && isFn(onfinally))
			args[1] = frameContinuation(onfinallySid, onfinally, true, true)
	}
}()
