...
let freeSwitchers = undefined
function getSwitcher(to, isPromiseJob = false, desc = '') {
	let switcher
	if(freeSwitchers) {
		switcher = freeSwitchers
		freeSwitchers = freeSwitchers.next
	}
	else switcher = function switcher() {
		const {to} = switcher
		switchTo(to)
		//if(isContinue) queuePromiseJob(getSwitcher(to), desc, true)
		if(isPromiseJob) queueJob(getSwitcher(to))
		switcher.to = undefined
		switcher.next = freeSwitchers
		if(freeSwitchers) freeSwitchers.next = switcher
	}
	switcher.to = to
	return switcher
}
...
function runIn(within, this_, fn, args) {
	...
	if(isFrameSwitch || isPromiseJob) {
		//queuePromiseJob(getSwitcher(within, isPromiseJob,`Continue ${name}`), `Enter ${name}-${within.id}`, true)
		queueJob(getSwitcher(within, isPromiseJob))
		switchTo(within)
	}

	let result
	let threw
	try {result = fn.call(this_, ...args)}
	catch(ex) {threw = ex}

	if(isFrameSwitch) {
		switchTo(from)
		//queuePromiseJob(getSwitcher(from), `Leave ${name}-${from.id}`)
		queueJob(getSwitcher(from))
	}
